(function () {
  ...
  const EVENT_CARDS = [
    { type: 'family', icon: '🫂', title: '사랑해 안아주기!', body: '가족을 꼬옥 안아주세요' },
    { type: 'family', icon: '✋', title: '하이파이브!', body: '옆 사람과 손바닥 짝!' },
    { type: 'mission', icon: '🦖', title: '공룡 흉내!', body: '멋진 공룡 소리를 내봐요' },
    { type: 'gift', icon: '🎁', title: '선물 팡!', body: '상자를 열어 보상을 받으세요' }
  ];
  ...
  async function showEvent(card) {
    ...
    const g = new PIXI.Graphics(); drawG(g, 'round', -200, -140, 400, 280, 30, 0xFFF5D8, 1, 6, 0xFFFFFF); cardFront.addChild(g);
  ...
  function drawRoulette() {
    ...
    const p = new PIXI.Graphics(); p.beginFill(0xFF2D55); p.lineStyle(6, 0xFFFFFF); p.moveTo(0,-174); p.lineTo(-30,-112); p.lineTo(30,-112); p.closePath(); p.endFill(); c.addChild(p);
